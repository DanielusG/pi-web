...
test("marks apply_patch returned failures as errors even when isError is unset", () => {
  ...
  // Fork theming: error surfaces use the --danger token instead of upstream's
  // hardcoded rgba(248,113,113,...) — same color in the dark theme, themed in light.
  assert.match(html, /border:1px solid color-mix\(in srgb, var\(--danger\) 40%, transparent\)/);
  assert.match(html, />apply_patch</);
  assert.doesNotMatch(html, /var\(--success\)/);
});
