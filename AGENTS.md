...
### Extension dialog is bottom-anchored (fork deviation from upstream)
Upstream renders the extension `ExtensionDialog` as a centered overlay (`alignItems: center`) over the message area. This fork keeps upstream's non-blocking overlay mechanics (pointer-events pass-through, input/stop stay alive, collapse pill, keyboard nav, countdown, `extension_ui_closed`, stop-unwinding) but **docks the expanded card at the bottom** of the message area, aligned to the chat column: the overlay wrapper uses `alignItems: "flex-end"` (expanded) and `padding: 12px ${CHAT_COLUMN_PADDING}px`, and the card uses `width: 100%; maxWidth: var(--chat-content-max-width, 820px)`. The collapsed pill stays top-anchored. Do not "fix" this back to centered when merging upstream — it is intentional. `ChatAppearance.test.mjs` expects 3 uses of `var(--chat-content-max-width, 820px)` in `ChatWindow.tsx` (the third is this dialog card).
