...
test("gzips a large JSON response when the client accepts gzip", async () => {
  const data = { value: "large payload ".repeat(200) };
  const response = jsonResponse(
    new Request("http://localhost/test", { headers: { "Accept-Encoding": "gzip" } }),
    data,
  );
  ...
  assert.ok(raw.length < Buffer.byteLength(JSON.stringify(data)), "gzip output must be smaller");
  assert.deepEqual(JSON.parse(gunzipSync(raw).toString("utf8")), data);
});
