...
for (const method of ["select", "confirm", "input", "editor", "custom"]) {
  const blocking = method !== "custom";
  test(`an open ${method} request ${blocking ? "marks" : "does not mark"} the session as waiting for input`, async (t) => {
    const { wrapper } = setup(t, async (ui) => {
      if (method === "custom") await ui.custom(() => ({ render: () => ["Choose"], dispose() {} }));
      else await ui[method]("Choose", method === "select" ? ["A", "B"] : "Details");
    });
    assert.equal(wrapper.hasPendingBlockingDialog(), false);
    const sending = wrapper.send({ type: "prompt", message: "/choose" });
    await nextTurn();
    assert.equal(wrapper.hasPendingBlockingDialog(), blocking);
    await wrapper.send({ type: "abort" });
    await sending;
    await nextTurn();
    assert.equal(wrapper.hasPendingBlockingDialog(), false);
