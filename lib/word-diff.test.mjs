...
test("highlights the whole changed word, not just the differing part", async () => {
  const { inlineDiff } = await loadSubject();
  const diff = inlineDiff("if (x > 10) {", "if (x > 20) {");

  assert.equal(oldTexts(diff), "if (x > 10) {");
  assert.equal(newTexts(diff), "if (x > 20) {");
  assert.equal(changedTexts(diff.old), "10");
  assert.equal(changedTexts(diff.new), "20");
});
...
test("handles unicode word characters", async () => {
  ...
  assert.equal(changedTexts(diff.old), "café");
  assert.equal(changedTexts(diff.new), "cafe");
});
